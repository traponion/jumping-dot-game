...
export const editorStore = createStore<EditorStore>()(
    devtools(
        immer((set, get) => ({
            ...
            // Core Actions
            selectTool: (tool: string) => set((state) => {
                state.editor.selectedTool = tool as any;
            }),
            
            setStageData: (stage: StageData | null) => set((state) => {
                state.stage = stage;
                if (stage) {
            ...
            updateEditorState: (updates: Partial<EditorState>) => set((state) => {
            ...
            showModal: (modal: string) => set((state) => {
            ...
            setSuccess: (success: string) => set((state) => {
            ...
            updateMousePosition: (position: { x: number; y: number }) => set((state) => {
                state.ui.mousePosition = position;
            }),
            
            // Performance Actions
            updatePerformance: (updates: Partial<PerformanceState>) => set((state) => {
...
export const useEditorStore = () => {
    throw new Error('useEditorStore is only available in React components. Use getEditorStore() instead.');
};
