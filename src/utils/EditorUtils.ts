...
export class TypeHelper {
     ...
     * 文字列を数値に安全に変換
     */
    static safeParseInt(value: string, defaultValue = 0): number {
     ...
     * 文字列を浮動小数点数に安全に変換
     */
    static safeParseFloat(value: string, defaultValue = 0): number {
     ...
     * オブジェクトから安全にプロパティを取得
     */
    static safeGetProperty<T, K extends keyof T>(obj: T, key: K, defaultValue: T[K]): T[K] {
...
export class MathHelper {
     ...
     * 2点間の距離を計算
     */
    static distance(point1: { x: number; y: number }, point2: { x: number; y: number }): number {
     ...
     * 2点間の角度を計算（度数）
     */
    static angle(point1: { x: number; y: number }, point2: { x: number; y: number }): number {
        const dx = point2.x - point1.x;
        const dy = point2.y - point1.y;
