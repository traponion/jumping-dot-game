...
**2025-06-19 更新**: 
**ECS（Entity-Component-System）アーキテクチャ復活プロジェクトは中止となりました。**

**新方針**:
- **既存OOPアプローチを継続**
- **300行制限を厳格に適用**（ESLint導入済み）
- **オニオンアーキテクチャは部分的に適用可能**
- 現在のゲーム規模ではECSはオーバーエンジニアリングと判断

## 🆕 新しい開発ロードマップ

### Phase 1: コード品質向上とリファクタリング
- [x] **ESLint 300行制限導入** ✅ 完了
- [ ] **長すぎるファイルの分割**（14ファイルが対象）
  - EditorController.ts (715行) → 最優先
  - FabricRenderSystem.ts (693行)
  - GameZustandStore.ts (603行)
  - その他11ファイル
- [ ] **既存Biome lintエラー修正**（272個）
- [ ] **部分的オニオンアーキテクチャ適用**

### Phase 2: 機能追加・改善
- [ ] ステージセレクト画面実装（issue #27）
- [ ] エディター機能改善（issue #26）
- [ ] 動く床実装（issue #2）
- [ ] その他オープンissue対応

### Phase 3: パフォーマンス・UX向上
- [ ] モバイル対応強化（issue #10）
- [ ] 難易度システム（issue #9）
- [ ] スローモーションモード（issue #12）

---

# ~~ECS・DDD・オニオンアーキテクチャ復活プロジェクト~~ - タスクロードマップ（廃止）
