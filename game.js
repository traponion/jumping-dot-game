class JumpingDotGame {
    ...
    update(deltaTime) {
        ...
        if (this.player.y > this.canvas.height + 100) {
            this.gameOver = true;
            this.gameStatus.textContent = 'Game Over - Press R to restart';
    ...
    checkSpikeCollisions() {
        for (const spike of this.stage.spikes) {
            if (this.player.x + this.player.radius > spike.x &&
                ...
                this.gameOver = true;
                this.gameStatus.textContent = 'Hit by spike! Press R to restart';
                // Clear trail on death
                this.trail = [];
                return;
            }
    ...
    checkHoleCollisions() {
        ...
        if (this.player.y > 600) {
            this.gameOver = true;
            this.gameStatus.textContent = 'Fell into hole! Press R to restart';
            // Clear trail on death
            this.trail = [];
        }
    }
